-- jitray, Cross-Platform, Modern, Handwritten, Updated LuaJIT bindings for raylib library.
-- Originally written by Rabia Alhaffar in 12/August/2020
-- Updated for jitray by Paul Furber 2024
...
local function load_error()
end

local raylib
local ok, err = xpcall(function() raylib = ffi.load(lib) end, load_error)
if not ok then
    error([[
Could not load the raylib shared library. 
]])
end
...
--rcamera.h constants
rl.CAMERA_CULL_DISTANCE_NEAR = rl.RL_CULL_DISTANCE_NEAR
rl.CAMERA_CULL_DISTANCE_FAR  = rl.RL_CULL_DISTANCE_FAR

rl.CAMERA_MOVE_SPEED                               = 0.09
rl.CAMERA_ROTATION_SPEED                           = 0.03
rl.CAMERA_PAN_SPEED                                = 0.2

-- Camera mouse movement sensitivity
rl.CAMERA_MOUSE_MOVE_SENSITIVITY                   = 0.003
rl.CAMERA_MOUSE_SCROLL_SENSITIVITY                 = 1.5

rl.CAMERA_ORBITAL_SPEED                            = 0.5


rl.CAMERA_FIRST_PERSON_STEP_TRIGONOMETRIC_DIVIDER  = 8.0
rl.CAMERA_FIRST_PERSON_STEP_DIVIDER                = 30.0
rl.CAMERA_FIRST_PERSON_WAVING_DIVIDER              = 200.0

rl.PLAYER_MOVEMENT_SENSITIVITY                     = 20.0
...
rl.AudioBuffer = function(...)
    return ffi.new("AudioBuffer", ...)
end

rl.AudioProcessor = function(...)
    return ffi.new("AudioProcessor", ...)
end
...
rl.FilePathList = function(...)
    return ffi.new("FilePathList", ...)
end

rl.AnimationEvent = function(...)
    return ffi.new("AnimationEvent", ...)
end

rl.AnimationEventList = function(...)
    return ffi.new("AnimationEventList", ...)
end
